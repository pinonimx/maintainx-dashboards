...
def calc_po_total(po):
    ...
    `price` = line total (unitCost × quantityOrdered), so we use
    ...
    for item in (po.get("items") or []):
        # priceDecimal is the line total as a dollar string (e.g. "146.200000")
        ...
        if line_str is not None:
            try:
                ...
                pass
        # Fallback: integer price field is in cents
...
def _title_redundant(title, vendor):
    ...
    if not t:
        return True
    # Exact match after normalizing, or one fully contains the other
...
def build_po_html(pos, generated_at):
    ...
    # Build vendor list for filter dropdown
  ...
  <tr>
    <td>
      ...
      <div style="text-align:center;padding:14px;font-size:11px;color:#94a3b8">
        ...
        For full interactive dashboard, open <em>po_dashboard.html</em>
      </div>
    </td>
  </tr>
</table>
